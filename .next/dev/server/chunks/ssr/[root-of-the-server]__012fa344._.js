...
const api = {
    ...
    claims: {
        list: async (page = 1, limit = 10, filter)=>{
            ...
            const params = filterParams ? `?${paginationParams}&${filterParams}` : `?${paginationParams}`;
            const response = await apiFetch(`/reclamo${params}`);
            return {
    ...
    statistics: {
        getResumen: async (fechaInicio, fechaFin)=>{
            ...
            return await apiFetch(`/reclamo/estadisticas/resumen${queryString}`);
        },
        getOverview: async ()=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__012fa344._.js.map